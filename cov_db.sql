drop database covdb;
drop user covv;
create user covv with password 'password';
create database covdb with template=template0 owner=covv;
\connect covdb


alter default privileges grant all on tables to covv;
alter default privileges grant all on sequences to covv;

create table cov_users(
...
foreign key (user_id) references cov_users (user_id);
...
create sequence cov_users_seq increment 1 start 1;
